...
public  class AbstractGenericDao<T,PK extends Serializable> {
	...
	@SuppressWarnings("unchecked")
	public List<T> getAll(String[] whereCauseAttr, Object[] whereCauseVal, String[] orderAttr, CommonType orderType){
		...
		try{
			...
			if(whereCauseAttr != null && whereCauseAttr.length > 0){
				if(whereCauseAttr.length == whereCauseVal.length){
					
					//===== concat where cause ====
					int len = whereCauseAttr.length;
					for(int i=0; i<len; i++){
						hql += "AND "+whereCauseAttr[i]+" = :param"+i+" ";
					}
					
					//===== concat order by ====
					if(orderAttr != null && orderAttr.length > 0){
						len = orderAttr.length;
						hql += "ORDER BY ";
						for(int i=0; i<len; i++){
							if(i > 0){
								hql += ",";
					...
					len = whereCauseAttr.length;
					for(int i=0; i<len; i++){
						query.setParameter("param"+i, whereCauseVal[i]);
					}
				}
			}
